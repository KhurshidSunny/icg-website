...
const ProductList = () => {
	...
	const filteredProducts = useMemo(() => {
		...
		if (filters.market) {
			result = result.filter(
				(product) =>
					product.industry_name?.toLowerCase() === filters.market.toLowerCase()
			);
		}
	...
	const totalPages = Math.max(
		1,
		Math.ceil(
			(filters.market || filters.category ? totalFilteredItems : total) / limit
		)
	);
	...
	return (
		<div className="max-w-6xl mx-auto p-4 bg-background-light dark:bg-background-dark">
			...
			{(filters.market || filters.category ? totalFilteredItems : total) >
				0 && (
